...
questions = [
    ("На сколько дней составить план питания? (от 1 до 30 дней)", "days"),
    ("На сколько человек нужно составить план питания? (Например: 2 взрослых и 1 ребёнок)", "servings"),
    ...
    ("Сколько времени в среднем вы готовы тратить на приготовление одного блюда? (Например: 30 минут)", "cooking_time"),
...
def generate_meal_plan(answers, days=1):
    ...
    prompt = (
        f"Создай разнообразный план питания на {answers['days']} дней для {answers['servings']} человек. Количество приёмов пищи в день: {answers['meals_count']}. "
        f"Пищевые ограничения и нелюбимые продукты: {answers['restrictions']}. "
        f"Максимальное время на приготовление одного блюда: {answers['cooking_time']}. "
        f"Бюджет: {answers['budget']}. "
        f"Желания по блюдам/продуктам: {answers['preferences']}. "
        "План должен включать указанные приёмы пищи на каждый день, с указанием ингредиентов, количества грам в порции и примерного времени приготовления. "
        "Обед должен содержать больше 1 блюда и напиток."
        "После плана питания, напиши список продуктов по данному плану с количеством конкретного продукта. (например, молоко - 1 бутылка, картофель - 1 кг) "
        "Каждый продукт в списке продуктов с новой строки с символом  •"
        "Не сокращай план или сообщение, если количество дней меньше 7."
        "Пример оформления ответа для 5 приемов пищи:"
        "📝 План питания"
        " "
        "🗓 День 1" 
        "🍳 Завтрак: [блюдо] ([ингредиенты])"
        "🥞 Второй завтрак: [блюдо] ([ингредиенты]) "  
        "🍲 Обед: [блюдо] ([ингредиенты])"
...
def edit_meal_plan(answers, current_plan, change_request):
    ...
    prompt = (
        f"У меня есть существующий план питания:\n\n{current_plan}\n\n"
        f"Пользователь хочет внести следующие изменения: {change_request}.\n\n"
        f"Также учитывай исходные параметры пользователя: количество приёмов пищи — {answers['meals_count']}, количество человек - {answers['servings']} "
        f"ограничения — {answers['restrictions']}, время на приготовление — {answers['cooking_time']}, "
        f"бюджет — {answers['budget']}, предпочтения — {answers['preferences']}.\n\n"
        "Внеси указанные изменения в план, сохранив его структуру и остальные блюда без изменений, если они не затрагиваются запросом. "
        "Ответ предоставь в том же структурированном формате на русском языке."
        "Пример оформления ответа для 5 приемов пищи:"
        "📝 План питания"
        " "
        "🗓 День 1" 
                ...
                if event.type == VkBotEventType.MESSAGE_NEW:
                    ...
                    if is_subscriber:
                        if user_id not in user_data:
                            ...
                            welcome_message = (
                                "Я ваш личный помощник по питанию.\n"
                                "Помогу составить персональное меню - чтобы не приходилось каждый день думать, что приготовить.\n"
